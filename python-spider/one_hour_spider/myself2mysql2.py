...
class downloader(object):
	...
	def __init__(self):
		...
		self.target = 'http://www.41tvtv.com:8888/news/other/17_'
		self.names = []			#存放章节名
		self.urls = []			#存放章节链接
		self.nums = 0			#章节数
	...
	def insert_tables(self,name, url, content):
		...
		# 使用cursor()方法获取操作游标
		cursor = conn.cursor()
		sql1 = """INSERT INTO city (title,url,content) VALUES('%s','%s','%s')""" % (name,url,content)
		# print(sql1)
		try:
			cursor.execute(sql1)
			# 执行sql语句
...
if __name__ == "__main__":
	...
	page=list(range(1,27))
	target=dl.target

	# dl.insert_tables('踢打发发送到','xxx', 'xwsh')
	# input('暂停')
	for i in page:
		dl.target=target+str(i+252)+'.html'
		print(dl.target)
		dl.get_download_url()
		print(dl.nums)
		print(dl.urls)
